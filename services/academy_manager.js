...
exports.pre_process_create_academy = async(req,resp)=>{
    const result = await  models.Academy.create({name: req.body.name,phone_number: req.body.phone_number,email: req.body.email,sports: req.body.sports}).then(function (academy) {
        if (academy) {
            resp.send(academy);
        } else {
