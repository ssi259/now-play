...
exports.update_plan = async(req,resp) =>{
    ...
    try{
        var input_response = await plan_manager.pre_process_update_plan(req)
        var processed_reponse =  await plan_manager.process_update_plan_input_req(input_response)
        var post_process_response = await plan_manager.post_process_plan_batch(resp,processed_reponse)
    }catch(e){
        console.log(e)
        const status_code = e.statusCode ? e.statusCode : 500
        return resp.status(status_code).send({ status: "Failure", message: e.name })
...
exports.getAllPlans = async (req, resp) => {
    try {
        var input_response = await plan_manager.pre_process_get_all_plans(req)
